...
$srcimg = $PRODUCT_URL . '/images/' . $imgRow['pimage'];

$collection =  explode('/', $_SERVER['REDIRECT_URL']);
$prodTitle = $SITE_NAME . ' ' . str_replace('_',' ',$collection[1]) . ' style ' . stripslashes($productsQryRow['sku']);
$text = stripslashes($productsQryRow['description']);
$prodText = str_ireplace('"', "'", strip_tags($text));
$actUrl = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
$shortUrl = short_url($actUrl);
?>
<body>
       ...
       <? include("left.php");?>
            ...
            <div class="clr"></div>
				...
				<? } ?>
                        ...
                        <?php if ($prevID != "") { ?><a id="goLeftAlter" href="<?= $prevID; ?>" title="<?= $prevID; ?>"></a><?php } ?>
                        <?php if ($nextID != "") { ?><a id="goRightAlter" href="<?= $nextID; ?>" title="<?= $nextID; ?>"></a><?php } ?>
                 </div>
                 <?php for($i=0;$i<6;$i++){?>
					<? $row=$productImgs[$i]; ?>
					<? if(!empty($row)){ ?>
                 ...
                 <ul id="thumblist">                 
				  ...
				  <p>
								  <?php if($colors_availText!=""){?>
				      <?php e_strt("Available colors");?> :
				      <?=$colors_availText;?>
				      <?php } ?>
				  </p>
			      </div>
			...
			<a target="_blank" href="http://pinterest.com/pin/create/button/?url=<?php echo $actUrl; ?>&media=<?php echo $srcimg; ?>&description=<?php echo $prodTitle; ?> <?php echo $prodText; ?>" class="pin-it-button" count-layout="horizontal"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" /></a>
                            ...
                            <div>
                                <a href="https://twitter.com/share" class="twitter-share-button" data-url="<?php echo $shortUrl; ?>" data-text="<?php echo $prodTitle; ?>" data-via="sinceritybridal" data-count="none">Tweet</a>
                ...
                */?>
                <?php if($productsQryRow['relateditems']!=""){
