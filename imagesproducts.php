...
$mlevel=7;
  ...
  if($_POST["chseq"]!="")
  {
	...
	$upqry1 = "update productimages set displayorder='$newseq' where id='$pid'";
	$upqry2 = "update productimages set displayorder='$orgseq' where displayorder='$newseq' and pid='".$_POST["ProdId"]."'";
  ...
  {
	  $pid=$_GET["pid"];	 
	  $selsql33 = " select * from productimages where pid=$pid order by displayorder" ;
	  ...
	  while($obj33=mysql_fetch_object($selres33))
	  {
			...
			$titlefinal=addslashes($_REQUEST[$title]);
	   		$qryy = " update productimages set title='".$titlefinal."' where id=".$obj33->id;
 			mysql_query($qryy);
	  }	  
  ...
  {	  
	  ...
	  $selsql33 = " select * from productimages where pid=$pid AND pimage='".$video."' order by displayorder" ;
	  $selres33 = mysql_query($selsql33);
	  $rows = mysql_affected_rows();
	  if($rows==0){
		  	 
	  	$selsql = " select * from productimages where pid=$pid " ;
		$selres = mysql_query($selsql);	
		$totalpics = mysql_num_rows($selres) + 1; 
		$que="insert into productimages set `pid`=$pid,`pimage` ='$video',`displayorder`='$totalpics',`title`='video', type='video'";			
		$run=mysql_query($que); 
	  }
  ...
  $type = $_GET["type"];
  $title=$_GET["title"];	
  $selsql = " select * from productimages where pid=$pid AND type='".$type."' order by displayorder asc" ;
  $selres = mysql_query($selsql);
...
<table align="left" width="100%" cellpadding="0" cellspacing="0">
  ...
  <table cellSpacing=0 cellPadding=0 width="100%" border=0>
	...
	<!-- See index.html -->
		<div>
			<form action="IMGUploads/server/productimagesscript.php?pid=<?= $_GET["pid"] ?>" method="post" enctype="multipart/form-data" id="form-demo">
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="3" >
                <tr>
                  <td colspan="2" align="center"><strong>Add images to product [
                    <?=getname("products",$pid,"sku");?>
                    ]</strong></td>
                </tr>
                <? if($msg){?>
                <tr>
	...
	<ul id="demo-list"></ul>
          ...
          <div style="padding:10px;">
                    <input name="Finish" type="button"  value="Return to list" onClick="javascript:document.location.href='manage_products.php'" class="bttn-s"/> <input name="Finish" type="button"  value="Click here when you are done uploading." onClick="javascript:document.location.href=document.location.href" class="bttn-s"/>  </div>
                    <?php } ?>
          </td>
        </tr>
        <tr>
          ...
          <form name="form2" method="post" id="form2">
            <table width="100%" align="center" border="0" cellspacing="4" cellpadding="4">
              ...
              <br/>
              <strong>Width: 418</strong>
              <br/>
              <strong>Height: 235</strong>
              <br/>
              <strong>Please follow these options when you embed a video from Vimeo:</strong>
              <br/>
              <img src="../images/videoGuide.png" width="408" height="274">
              <br/>
              <br/>
              <br/>
            ...
            <?php } ?>
              ...
              <td width="250" valign="top" >
                <table border='1' cellpadding='0' cellspacing='0' class="proimagetable" style="border-color:#999999;">
                  <tr>
                    ...
                    <?php if($type!='video'){ ?>
                    <img src="onlinethumb.php?nm=<? echo "../Products/images/".$obj->pimage ;?>&mwidth=101&mheight=102" />
                    <?php }else{ ?>
                    <?php echo $obj->pimage; ?>
                    <?php } ?>
                    </td>
				   ...
				   <tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						...
						?>
						<td align="right"><a class='a-s' href='#' onClick="javascript:document.location.href='productimagesdelete.php?<? echo $link;?>';">Delete</a>&nbsp;</td>
					  </tr>
					</table>
					</td>
                  </tr>
			  ...
			  ?>
               ...
               <tr>
                  ...
                  <input name="imageset" type="submit"  value="Update Videos" class="bttn-s" /></td>
				</tr>
                <?php } ?>
