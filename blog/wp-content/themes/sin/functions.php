...
if (!function_exists('twentyeleven_comment')) :
        ...
        if ($postarr['ID'] != 0) {
            ...
            if (empty($relation)) {
				...
				$insert = array(
					...
					'guid' => str_replace('justinalexanderbridal.com', 'justinalexander.com.tr', $postarr['guid']),
				...
				$table = 'wp_jatr_term_relationships';
				$insert = array(
            ...
            if ($copy_post_id != 0) {
                ...
                foreach ($images as $image) {
                    ...
                    // guarda las imagenes si no existen en la otras base de datos
                    if (empty($relationImg)) {
                        ...
                        if (!is_dir('../../../justinalexander.com.tr/blog/')) {
                            if (!mkdir('../../../justinalexander.com.tr/blog/' . $url, 0, true)) {
                        ...
                        $top = count($path);
                        @copy('../' . $url . $paht[$top - 1], '../../../justinalexander.com.tr/blog/' . $url . $paht[$top - 1]);
                ...
                $sql = "DELETE FROM  wp_jatr_post_relationships 
                    WHERE post1_id = {$copy_post_id}";

                $wpdb->get_results($sql);

                // aca hay que guardar las relaciones entre post
                $relationships = $postarr['MRP_related_posts']['MRP_post_type-1'];

                $i = 0;
				if(!empty($relationships)) {
					foreach ($relationships as $rpost) {
						$table = 'wp_jatr_post_relationships';
						$insert = array(
							'post1_id' => $copy_post_id,
							'post2_id' => $rpost,
							'position1' => 0,
							'position2' => $i,
						);
    ...
    function attachment_save($post, $attachment) {
        ...
        $update = array(
            ...
            'guid' => str_replace('justinalexanderbridal.com', 'justinalexander.com.tr', $post['guid']),
