 ...
 Delicious
	...
	function dcssb_inc_pinit($link){
		...
		if($this->get_dcssb_default('incPinit')){
			...
			$typeclass = $method == 'featured' ? ' pin-featured' : ' pin-preview' ;
			$button = '<li id="dcssb-pinit" class="'.$classSize.$typeclass.'">';
			...
			<script type="text/javascript">
				...
				<?php if($method == 'stick'){ ?>
					...
					if(!jQuery().dcSocialSlick) {
						jQuery.getScript('<?php echo dc_jqslicksocial::get_plugin_directory(); ?>/js/jquery.social.slick.1.0.js', function(){
				...
				<?php } else { ?>
					...
					if(!jQuery().dcSocialFloater) {
						jQuery.getScript('<?php echo dc_jqslicksocial::get_plugin_directory(); ?>/js/jquery.social.float.1.3.js', function(){
