...
define ( 'FBCONNECT_PLUGIN_REVISION', 136); 

define ( 'FBCONNECT_DB_REVISION', 18);
...
define('WP_CONTENT_URL_FB', get_option('siteurl') . '/wp-content');

if (! defined('WP_THEME')){
	define('WP_THEME', get_option('siteurl') . '/wp-content/themes');
}
define('WP_THEME_FB', get_option('siteurl') . '/wp-content/themes');
...
class WPfbConnect {
	...
	function log($msg,$level=1){
		//echo "LEVEL:".$level." ".FBCONNECT_LOG_LEVEL;
		if (FBCONNECT_LOG_LEVEL!=-1 &&FBCONNECT_LOG_LEVEL!="" && $level<= FBCONNECT_LOG_LEVEL){
			...
			//echo $msg;
			//echo FBCONNECT_PLUGIN_PATH_LOG;
...
function fb_remove_admin_bar(){
	if (get_option('fb_removeadminbar') && current_user_can('subscriber')){
...
function fb_wp_login_form() {
	...
	echo '<div style="margin-top:1px;margin-bottom:5px;">Or login with Facebook:</div>';
	...
	<a class="fb_button fb_button_large" onclick="login_facebookjs('<?php echo $url;?>')">
		...
		if($_REQUEST['fbconnect_action']=="community"){
			...
			if(function_exists('wp_redirect')){
				if (isset($_SERVER["HTTP_REFERER"]) && $_SERVER["HTTP_REFERER"]!=""){
					...
					wp_redirect( get_option('siteurl')."?fbconnect_action=postlogout" );
				}
			}
...
include( ABSPATH.'wp-admin/includes/user.php');
		...
		if ( current_user_can('subscriber')){
			...
			if (  isset($user) && $user!="" && $fb_user!="" && $user->fbconnect_userid == $fb_user){
				...
				//$url =  wp_logout_url( get_option('siteurl') ); 
...
function widget_FacebookConnector($args) {
	...
	function widget_FacebookConnector_control() {
		...
		if ( $_POST['FacebookConnector-submit'] ) {
			...
			$options['avatarsize'] = (int)$_POST['FacebookConnector-avatarsize'];
		...
		$avatarsize = htmlspecialchars($options['avatarsize'], ENT_QUOTES);
		//get_option('blogname')
		...
		echo '<p style="text-align:right;"><label for="FacebookConnector-avatarsize">'.__('Users photos size:', 'fbconnect').' <input style="width: 180px;" id="FacebookConnector-avatarsize" name="FacebookConnector-avatarsize" type="text" value="'.$avatarsize.'" /></label></p>';
...
function widget_FBConnector_CommentsFeed($args) {
		...
		global $fbconnect_filter;
		global $maxcomments;
		$maxcomments = $maxlastcomments;
		global $fbconnect_avatarsize;
		$fbconnect_avatarsize = $avatarsize;
		global $fbconnect_showusername;
		$fbconnect_showusername = $showusername;
