...
//					var_dump($pressImgs);
                ...
                <div class="pressPage">
					...
					$top = count($pressImgs);
					for($i = 2; $i < $top; $i++) {?>                   
                    <a class="pressImg pressImg<?=$count?>" href="<?=$PRESS_URL;?>/<?=$pressImgs[$i]['pimage'];?>" title="<?=stripslashes($row['title']);?>">
                    <img src="<?=$PRESS_URL;?>/<?=$pressImgs[$i]['pimage'];?>"  title="<?=stripslashes($row['title']);?>" />
                    </a>
                    <?php } ?>
                    </div>
                    <p>
                    ...
                    <?=$text?>
                        <?php //stripslashes($row['short_desc']);?>
                            ...
                            <!--este a estaba dentro del if -->
                    <?php if($row['pdf']!="" | $row['url'] != "" ){?>                                  
                         <?php if(stripos($row['url'], 'http') !== false) {//then link directly to the url ?>
				...
				<?php } ?> 
                <?php //include("right_facebook.php");?>
